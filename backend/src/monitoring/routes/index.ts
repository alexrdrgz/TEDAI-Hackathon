...
import { addSnapshot, getSessionSnapshots, getLastSessionSnapshot, getSessionTimeline, updateSessionTimeline } from '../models/database';
    ...
    pythonProcess.on('close', async (code) => {
      if (code === 0) {
        try {
          ...
          const previousSnapshot = getLastSessionSnapshot(sessionId);
          ...
          // Store in database
          addSnapshot(filePath, summary.Caption, summary.FullDescription, summary.Changes, summary.Facts, sessionId);
          
          // Generate and append timeline entry
          const currentTimeline = getSessionTimeline(sessionId);
          ...
          updateSessionTimeline(sessionId, updatedTimeline);
